...
class Home{
	...
	delayOn(name, delay){//  РАБОТАЕТ
		return new Promise( (resolve, reject) => {
			setTimeout( () => {
				resolve(myHome.getDeviceByName(name).on());
				//reject("Ошибка!");
	...
	/*delayOn(name, delay, callback){//airBedroom - 1й без задержки сразу вкл !!!? в остальном  РАБОТАЕТ
		myHome.getDeviceByName(name).on();// myHome.???
...
myHome.delayOn("airBedroom", 2000)
	.then(		
		myHome.delayOn("Kitchen1", 3000)
	)
	.then(
		myHome.delayOn("airBedroom2", 4000)
	)
	.then(
		myHome.delayOn("washingMachine", 5000)
	)	
	/*.then(	() => {	
		myHome.delayOn("Kitchen1", 3000)
		},
...
/*myHome.delayOn("airBedroom", 4000, function(){//работ
...
console.log(myHome.getDeviceByName("airBedroom").status);// true сразу
